...
class StatWalletAdapter(private val context: Context?, private val arrayList: ArrayList<StatWallet>) : BaseAdapter() {
    ...
    override fun getView(position: Int, convertview: View?, parent: ViewGroup?): View {
        ...
        imgLink = convertView.findViewById(R.id.item_stat_imgWallet)
        ...
        Glide.with(context!!).load(arrayList[position].imgLink).into(imgLink)

        //change color of balance
        if(arrayList[position].saldo > 0) saldo.setTextColor(Color.parseColor("#00752F"))
        else saldo.setTextColor(Color.parseColor("#F44336"))
